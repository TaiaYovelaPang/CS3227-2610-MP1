...
To reset the workspace, close TeamSync and rename, move, or delete `.teamsync-mvp-workspace.ser`. Keep a copy first if you may need to recover the data. On the next start, TeamSync creates an otherwise-empty workspace with this release's bundled example sheet URL.
